...
class Network(nn.Module):
    def __init__(
        ...
        #self.bottam_layer_1 = nn.Conv2d(in_channels=512, out_channels=256, kernel_size=3, stride=1, padding=1)
        ...
        for i in range(len(dim_mults)-1): # each dblock
            ...
            self.down.append(
                nn.ModuleList([
                    ...
                    FusionMamba(dim=dim_out, depth=1, final=True , dropout_prob=0.29)
    ...
    def forward(self, seg, img, time):
        ...
        #p=x
        w=self.multis(p) #消融实验：不使用多尺度融合模块
        x=torch.cat((z,attn_output,w), dim=1)
        #x=torch.cat((z,attn_output), dim=1)  #消融实验：不使用多尺度融合模块
        x=self.bottam_layer(x)
        #x=self.bottam_layer_1(x)  #消融实验：不使用多尺度融合模块
        #x=x+p
