...
from recursivediffusion.recursive_noise_diffusion.networks.network import Network, NetworkConfig
from recursivediffusion.recursive_noise_diffusion.utils.cityscapes_loader import CityscapesLoader
from recursivediffusion.recursive_noise_diffusion.utils.evaluation import Evaluator
from recursivediffusion.recursive_noise_diffusion.utils.pascal_voc_loader import PascalVOCLoader
from recursivediffusion.recursive_noise_diffusion.utils.trainer import TrainerConfig
from recursivediffusion.recursive_noise_diffusion.utils.utils import set_seed
from recursivediffusion.recursive_noise_diffusion.utils.uavid_loader import UAVidLoader
from recursivediffusion.recursive_noise_diffusion.utils.vaihingen_buildings_loader import VaihingenBuildingsLoader

from recursivediffusion.recursive_noise_diffusion.utils.isicloarers import ISICDatasetLoader
from recursivediffusion.recursive_noise_diffusion.utils.refugeloader import refugeDatasetLoader
from recursivediffusion.recursive_noise_diffusion.utils.monuseg_loader import MoNuSegDatasetLoader
from recursivediffusion.recursive_noise_diffusion.utils.glas_loader import GlasDatasetLoader
...
def make_parser():
    ...
    parser.add_argument("--scale_procedure", metavar='SP', type=str, action="store",
                        default=NetworkConfig.scale_procedure,
...
def main():
    ...
    if ARGS.dataset_selection == "cityscapes":
        ...
        test_dataset = VaihingenBuildingsLoader(root='/exp/home/mingkai.liu/data/Vaihingen_buildings', split='test',
                                                is_transform=True)
    elif ARGS.dataset_selection == "uavid":
        test_dataset = UAVidLoader(root=datas, split='val', is_transform=True)
    elif ARGS.dataset_selection == "isic":
        # test_dataset = ISICDatasetLoader(root=datas, split='val', is_transform=True)
        test_dataset = ISICDatasetLoader(root='/exp/home/mingkai.liu/data/ISIC', split="test", img_size=256,
                                         is_transform=True, img_norm=True)
    elif ARGS.dataset_selection == "refuge":
        test_dataset = refugeDatasetLoader(root=datas, split="test", img_size=256, is_transform=True, img_norm=True)
    elif ARGS.dataset_selection == "monuseg":
        test_dataset = MoNuSegDatasetLoader(root=datas, split="test", img_size=512, is_transform=True, img_norm=True)
    elif ARGS.dataset_selection == "glas":
        test_dataset = GlasDatasetLoader(root=datas, split="test", img_size=512, is_transform=True, img_norm=True)

    # define dataset loader
    test_dataloader = DataLoader(test_dataset, batch_size=ARGS.batch_size, shuffle=False, num_workers=ARGS.n_workers)
    # test_dataloader = DataLoader(test_dataset, batch_size=1, shuffle=False, num_workers=ARGS.n_workers)
    ...
    dummy_input = torch.randn(1, 2, 224, 224).to(device)
    image_input = torch.randn(1, 3, 224, 224).to(device)
    #t=torch.tensor([0.9600]).to(device)
    t = torch.tensor([0.0133]).to(device)
    flops, params = profile(model, inputs=(dummy_input,image_input,t))
