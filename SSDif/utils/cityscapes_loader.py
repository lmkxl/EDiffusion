...
class CityscapesLoader(data.Dataset):
    ...
    def __init__(
        ...
        self.n_classes = 19
        self.ignore_index = 255 #忽略索引的值，设置为 255，用于标记无效像素或背景
        self.img_size = img_size if isinstance(img_size, tuple) else (img_size, img_size)
        self.mean = torch.tensor([0.485, 0.456, 0.406])
        self.std = torch.tensor([0.229, 0.224, 0.225])
        self.files = {}

        self.images_base = os.path.join(self.root, "leftImg8bit", self.split) #构建图像数据的根目录路径，将 self.root、leftImg8bit 和 split 组合成路径，存放对应划分的图像文件
        self.annotations_base = os.path.join(self.root, "gtFine", self.split) #构建标签数据的根目录路径，将 self.root、gtFine 和 split 组合成路径，用于存放对应划分的标签文件。
        self.files[split] = self.recursive_glob(rootdir=self.images_base, suffix=".png")#使用 recursive_glob 方法获取 self.images_base 目录下所有以 .png 结尾的文件路径，返回的文件路径列表存储在 self.files 字典中

        self.void_classes = [0, 1, 2, 3, 4, 5, 6, 9, 10, 14, 15, 16, 18, 29, 30, -1] #列出无效类别的标签 ID，这些类别将被忽略，不参与训练或评估
        self.valid_classes = [
            7,
            8,
            11,
            ...
            33,
            self.ignore_index,
        ]#列出有效类别的标签 ID，并包括 self.ignore_index，这些类别会被用来训练或评估。
        self.class_names = [
            "road",
            "sidewalk",
            "building",
            ...
            "bicycle",
            "unlabelled",
        ]#列出有效类别的名称，与 self.valid_classes 一一对应，用于描述每个有效类别。
