...
class MoNuSegDatasetLoader(data.Dataset):
...
if __name__ == "__main__":
    root = "path/to/monuseg/dataset"  # Root directory of the MoNuSeg dataset
    train_dataset = MoNuSegDatasetLoader(root=root, split="train", img_size=(256, 256), is_transform=True)
    test_dataset = MoNuSegDatasetLoader(root=root, split="test", img_size=(256, 256), is_transform=True)

    train_loader = data.DataLoader(train_dataset, batch_size=8, shuffle=True, num_workers=4)
    test_loader = data.DataLoader(test_dataset, batch_size=8, shuffle=False, num_workers=4)
